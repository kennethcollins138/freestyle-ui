...
export class AppController {
  ...
  async editElement(
    pageId: string,
    elementId: string,
    updates: Partial<ElementSchema>,
  ): Promise<PageSchema | HomeSchema> {
    const appInstance = await this.loadAppInstance();

    if (appInstance) {
      const page =
        pageId === "home" ? appInstance.home : appInstance.pages[pageId];
      if (page) {
        const elementIndex = page.children.findIndex(
          (el: ElementSchema) => el.id === elementId,
        );
        if (elementIndex !== -1) {
          page.children[elementIndex] = {
            ...page.children[elementIndex],
            ...updates,
          };
          await this.saveAppInstance(appInstance);
  ...
  async deleteNode(
    pageId: string,
    elementId: string,
  ): Promise<PageSchema | null | HomeSchema> {
    const appInstance = await this.loadAppInstance();
    if (appInstance) {
      const page =
        pageId === "home" ? appInstance.home : appInstance.pages[pageId];
   ...
   */
  async addOrUpdateImageData(
    componentId: string,
    imageData: ImageElementSchema,
  ): Promise<void> {
    const appInstance = await this.loadAppInstance();
    if (appInstance) {
   ...
   */
  async getImageDataByComponentId(
    componentId: string,
  ): Promise<ImageElementSchema | null> {
    const appInstance = await this.loadAppInstance();
  ...
  async reorderElements(
    pageId: string,
    elementOrder: string[],
  ): Promise<PageSchema | HomeSchema | null> {
    const appInstance = await this.loadAppInstance();
    if (appInstance && appInstance.pages[pageId]) {
      const page = appInstance.pages[pageId];
