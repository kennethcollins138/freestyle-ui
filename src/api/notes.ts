...
export class AppController {
  ...
  async getAppInstance(): Promise<AppInstance> {
    ...
    const appInstance = await rdx.appInstanceGet(this.postId);
    console.log('Retrieved app instance:', appInstance); // Added logging
    return appInstance;
  }
  ...
  async createAppInstance(params: Omit<AppInstance, 'createdBy'>, username: string): Promise<AppInstance> {
    console.log('Creating app instance with params:', params);
    const appInstance: AppInstance = {
      ...params,
      createdBy: username,
    };
    const rdx = new Redis(this.context.redis);
    const createdInstance = await rdx.appInstanceCreate(this.postId, appInstance);
    console.log('Created app instance:', createdInstance); // Added logging
