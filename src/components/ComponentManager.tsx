...
export class ComponentManager {
     ...
     * @param navigateCallback Callback for navigation actions
     * @returns JSX element for the component
     */
    renderComponent = (component: ElementSchema, navigateCallback?: (route: string, params: any) => void) => {
        if (!component.type) {
        ...
        switch (component.type) {
            case 'VStack':
                return (
                    <VStackElement key={component.id} {...component}>
                        {component.children?.map((child: ElementSchema) => this.renderComponent(child, navigateCallback))}
                    </VStackElement>
                );
            case 'HStack':
                return (
            ...
            case 'PaginationButton':
                ...
                if (navigateCallback && component.pageId) {
                    return (
                        <button
                            key={component.id}
                            icon={component.icon}
                            ...
                            appearance={component.appearance}
                            onPress={async () => navigateCallback('pagination', { pageId: component.pageId })}
                            {...(component.width ? { width: component.width } : {})}
                            {...(component.height ? { height: component.height } : {})}
                            {...(component.isGrow ? { grow: true } : {})}
                        >
            ...
            case 'PersonalPlug':
                return (
                    <hstack key={component.id} gap="small" alignment="center bottom" width="100%">
                        <icon name="link" onPress={async () => { this.context.ui.navigateTo('https://www.reddit.com/user/TheRepDeveloper/')}}>
                            {component.content || 'Created by /u/TheRepDeveloper'}
                        </icon>
                    </hstack>
                );
            default:
     ...
     */
    updateComponentRecursive(elements: ElementSchema[], editedComponent: ElementSchema): ElementSchema[] {
     ...
     */
    deleteComponentRecursive(elements: ElementSchema[], componentId: string): ElementSchema[] {
        return elements
            .map((el) => {
                if (el.children && el.children.length > 0) {
    ...
    addComponentToParentRecursive(
        ...
        return elements.map((el) => {
            ...
            if (el.children && el.children.length > 0) {
                return {
                    ...el,
                    children: this.addComponentToParentRecursive(el.children, parentId, newComponent),
                };
            }
            return el;
        });
     ...
     */
    findComponentById(elements: ElementSchema[], componentId: string): ElementSchema | undefined {
     ...
     */
    showComponentCreator(componentType: string, onSave?: (component: ElementSchema) => void) {
            ...
            async (formData) => {
                ...
                switch (componentType) {
                    ...
                    case 'Image':
                        ...
                        await this.appController.addOrUpdateImageData(newComponent.id, newComponent);
                        break;
                    case 'VStack':
                    ...
                    default:
                        ...
                        this.context.ui.showToast(`Unknown component type: ${componentType}`);
                        return;
                }
     ...
     * @param onSave Callback for when the component is saved
     */
    showComponentEditor(component: ElementSchema, onSave?: (component: ElementSchema) => void) {
        const editor = getComponentEditor(
            this.context,
            component.type,
            component,
            async (formData) => {
                ...
                switch (component.type) {
                    ...
                    case 'Image':
                        ...
                        // Handle image uploads specially
                        await this.appController.addOrUpdateImageData(editedComponent.id, editedComponent);
                        break;
                    case 'VStack':
                    ...
                    default:
                        ...
                        this.context.ui.showToast(`Unknown component type: ${component.type}`);
                        return;
                }
    ...
    async updateComponentOnHome(component: ElementSchema): Promise<HomeSchema> {
        ...
        const updatedHome = deepClone(home) as HomeSchema;
        updatedHome.children = this.updateComponentRecursive(updatedHome.children, component);
        await this.appController.updateAppInstance({ home: updatedHome });
    ...
    async deleteComponentFromHome(componentId: string): Promise<HomeSchema> {
        ...
        const updatedHome = deepClone(home) as HomeSchema;
        updatedHome.children = this.deleteComponentRecursive(updatedHome.children, componentId);
        await this.appController.updateAppInstance({ home: updatedHome });
     ...
     */
    async addComponentToPage(pageId: string, component: ElementSchema): Promise<PageSchema | HomeSchema> {
        if (pageId === 'home') {
     ...
     * @returns Promise resolving to the updated page structure
     */
    async updateComponentOnPage(pageId: string, component: ElementSchema): Promise<PageSchema | HomeSchema> {
        if (pageId === 'home') {
        ...
        const updatedPage = deepClone(page) as PageSchema;
        updatedPage.children = this.updateComponentRecursive(updatedPage.children, component);
        await this.appController.savePage(pageId, updatedPage);
     ...
     * @returns Promise resolving to the updated page structure
     */
    async deleteComponentFromPage(pageId: string, componentId: string): Promise<PageSchema | HomeSchema> {
        if (pageId === 'home') {
        ...
        const updatedPage = deepClone(page) as PageSchema;
        updatedPage.children = this.deleteComponentRecursive(updatedPage.children, componentId);
        await this.appController.savePage(pageId, updatedPage);
     ...
     * @returns Promise resolving to the updated page structure
     */
    async addChildComponent(pageId: string, parentId: string, component: ElementSchema): Promise<PageSchema | HomeSchema> {
        if (pageId === 'home') {
            const home = await this.appController.readWholePage('home');
            if (!home) {
                throw new Error('Home page not found');
            ...
            const updatedHome = deepClone(home) as HomeSchema;
            updatedHome.children = this.addComponentToParentRecursive(updatedHome.children, parentId, component);
            await this.appController.updateAppInstance({ home: updatedHome });
        ...
        const updatedPage = deepClone(page) as PageSchema;
        updatedPage.children = this.addComponentToParentRecursive(updatedPage.children, parentId, component);
        await this.appController.savePage(pageId, updatedPage);
     ...
     */
    async createNewPageWithButton(pageId: string, buttonText: string): Promise<string> {
